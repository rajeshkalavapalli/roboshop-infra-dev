...
resource "aws_cloudfront_distribution" "bigmatrix" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.bigmatrix.domain_name
        zone_id = aws_cloudfront_distribution.bigmatrix.hosted_zone_id
      }
    }
